...
def train(model, train_loader, test_loader, criterion, optimizer, epochs, model_save_path):
    ...
    for epoch in range(epochs):
        ...
        for traces, labels in train_loader:
            traces, labels = traces.to(device, dtype=torch.float32), labels.to(device)
...
class TraceDataset(Dataset):
    """Custom Dataset class for website fingerprinting trace data."""
    ...
    def __init__(self, json_path, input_size):
        ...
        for entry in dataset_json:
            ...
            # Pad or truncate trace data to fixed input size
            ...
            self.labels.append(website_index)
            self.website_names_dict[website_index] = website_name

        self.data = np.array(self.data, dtype=np.float32)
        self.labels = np.array(self.labels, dtype=np.int64)

        self.website_names = [self.website_names_dict[i] for i in sorted(self.website_names_dict.keys())]
...
def main():
    """Full pipeline using custom Dataset class."""

    # 1. Load dataset
    dataset_all = TraceDataset(DATASET_PATH, INPUT_SIZE)

    # 2. Split into train/test indices using stratified sampling
    stratified_split = StratifiedShuffleSplit(n_splits=1, test_size=(1 - TRAIN_SPLIT), random_state=42)
    idx_all = list(range(len(dataset_all)))
    y_labels = dataset_all.labels

    for idx_train, idx_test in stratified_split.split(idx_all, y_labels):
        subset_train = Subset(dataset_all, idx_train)
        subset_test = Subset(dataset_all, idx_test)

    # 3. DataLoaders
    loader_train = DataLoader(subset_train, batch_size=BATCH_SIZE, shuffle=True)
    loader_test = DataLoader(subset_test, batch_size=BATCH_SIZE, shuffle=False)

    n_classes = len(dataset_all.website_names)
    print(f"Loaded dataset with {len(dataset_all)} samples and {n_classes} classes.")

    # 4. Model definition
    model_save_path = os.path.join(MODELS_DIR, "best_model.pth")
    model = ComplexFingerprintClassifier(INPUT_SIZE, HIDDEN_SIZE, n_classes)
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    criterion = nn.CrossEntropyLoss()

    # 5. Train
    best_acc = train(model, loader_train, loader_test, criterion, optimizer, EPOCHS, model_save_path)
    print(f"\nTraining complete. Best test accuracy: {best_acc:.4f}")

    # 6. Evaluation
    model.load_state_dict(torch.load(model_save_path))
    evaluate(model, loader_test, dataset_all.website_names)
